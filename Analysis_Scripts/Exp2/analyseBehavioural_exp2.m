%% Analyse Behavioural Data - EXP 2

close all;
clear all;
clc;

dir_data = 'Data\Exp2\Behavioural'; % location of raw data files (e.g. s1_trial_info.mat)

subjects            = 1:33; %setdiff(1:33,[3 23]); % remove S03 and S23
...
numtrials = 90; % trials per block
numblocks = 14; % total number of blocks

block_include = 1:14; % blocks to analyse (i.e. exclude first 2 for learning)
plot_subjects = 0;

% mean centre STAI based on subject selection above (line 9)
load(fullfile(dir_data,'stai.mat')); % load state-trait anxiety scores
stai.meancentred_full = mean(stai.full) - stai.full;
stai.meancentred_state = mean(stai.state) - stai.state;
stai.meancentred_trait = mean(stai.trait) - stai.trait;
...
%     
    ...
    nblocktrials = repmat(1:numtrials,1,numblocks)';
    nblockcount = repmat(1:numblocks,numtrials,1);
    nblockcount = nblockcount(:);
    nblockcount = nblockcount(~outliers & correct_responses,1);
    
    ntrials = sum(~outliers & correct_responses);
    ntrials_idx = [1:(numblocks*numtrials)]';
...
dlmwrite('trial_data_exp2.csv',trial_data,'-append');
