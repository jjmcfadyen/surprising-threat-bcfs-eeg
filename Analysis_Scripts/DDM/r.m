...
% 
% theta: parameter vector [a, Ter (sec), eta, St(sec), v, s]
...
function p = r(RT,theta)
    ...
    samples = size(RT,1);
    t1 = RT - repmat(theta(2)+theta(4),samples,1); % non-decision time MINUS the SD of non-decision time
    t2 = RT - repmat(theta(2)-theta(4),samples,1);
    
    S1 = ones(samples,1);
    S2 = ones(samples,1);
    
    t1v = t1(t1>0); % remove negative values
    t2v = t2(t2>0);
